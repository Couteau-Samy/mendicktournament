    ...
    const results = await Promise.all(
      players.map(async (p) => {
        try {
          ...
          // 📊 SOUSTRACTION DES GAMES AVANT CHALLENGE
          ...
          if (currentAbs > startAbs && startIndexTier !== -1 && currentIndexTier !== -1) {
            ...
            tierBonus = tiersCrossed * 150; // [cite: 25]
            ...
            divisionBonus = divisionsCrossed * 50; // [cite: 24]
          }

          // Points de Winrate (Seulement le winrate du challenge * 20)
          const winratePoints = challengeWinrate * 20; // [cite: 19]

          const finalScore = winratePoints + lpGainPoints + divisionBonus + tierBonus;

          return {
            name: p.name,
            wins: challengeWins, // On renvoie uniquement les stats du challenge au front
            ...
            scoreDetails: {
              ...
              finalScore: challengeTotalGames >= 30 ? finalScore : 0, // Validation du quota des 30 games minimum
              isInvalid: challengeTotalGames < 30 // [cite: 33]
            }
          };
        } catch (err) {
          return { name: p.name, wins: 0, losses: 0, totalGames: 0, lp: 0, tier: "ERROR", rank: "", winrate: 0, startDisplay: "Inconnu", scoreDetails: { winratePoints: 0, lpGainPoints: 0, bonusPoints: 0, finalScore: 0, isInvalid: true } };
        }
      })
    );
